  ...
  return (
    <div className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700 hover:border-primary-500/50 transition-all duration-300 shadow-lg hover:shadow-primary-500/10 flex flex-col h-full">
      <div className="p-6 md:p-8 flex flex-col flex-grow">
        ...
        <div className="mt-8 pt-6 border-t border-slate-700">
          ...
          <div className="flex flex-wrap gap-2">
            {project.technologies.map((tech, idx) => (
              <span key={idx} className="px-3 py-1 bg-slate-900 text-primary-400 text-xs font-medium rounded-full border border-slate-700">
                {tech}
              </span>
            ))}
          </div>
        ...
        {project.githubUrl && (
          <a href={project.githubUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-900/80 hover:bg-slate-900 text-slate-200 text-sm font-semibold rounded-lg border border-slate-700 hover:border-primary-500/50 transition-all group">
            <Github size={16} />
            View on GitHub
